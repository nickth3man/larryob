...
def test_player_game_log_row_raises_when_fgm_exceeds_fga() -> None:
    with pytest.raises(ValidationError, match="fgm > fga"):
        PlayerGameLogRow(
            game_id="001", player_id="1", team_id="2",
            fgm=15, fga=10,
        )


def test_player_game_log_row_raises_when_fg3m_exceeds_fg3a() -> None:
    with pytest.raises(ValidationError, match="fg3m > fg3a"):
        PlayerGameLogRow(
            game_id="001", player_id="1", team_id="2",
            fg3m=5, fg3a=3,
        )
...
def test_player_game_log_row_raises_when_ftm_exceeds_fta() -> None:
    with pytest.raises(ValidationError, match="ftm > fta"):
        PlayerGameLogRow(
            game_id="001", player_id="1", team_id="2",
            ftm=7, fta=5,
        )
...
def test_player_game_log_row_raises_when_pts_negative() -> None:
    with pytest.raises(ValidationError, match="pts < 0"):
        PlayerGameLogRow(
            game_id="001", player_id="1", team_id="2",
            pts=-1,
        )
...
def test_player_game_log_row_raises_when_reb_sum_mismatch() -> None:
    with pytest.raises(ValidationError, match="oreb.*dreb.*reb|oreb \\+ dreb"):
        PlayerGameLogRow(
            game_id="001", player_id="1", team_id="2",
            oreb=2, dreb=3, reb=10,
        )
...
def test_player_game_log_row_passes_when_all_nullable_fields_are_none() -> None:
    ...
    row = PlayerGameLogRow(
        ...
        reb=None, pts=None, minutes_played=None,
    )
    assert row.fgm is None
